#!/usr/bin/env python3
"""Frozen native RGB inference with sensor intrinsics supplied directly to the network."""
...
VARIANTS={'native896_sensorK':dict(size=512,native=True,reset=False)}
...
def main():
 ...
 calibrations=json.loads((rich/'processed/dataset_v1/cameras.json').read_text())['cameras']
 provenance=dict(frontend_revision=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
  ...
  frozen=True,geometry_only=True,GT_body_or_contact_input=False,sensor_intrinsics_supplied_to_model=True,sensor_helper_sha256=sha256(ROOT/'contact_streaming/rich_sensor_intrinsics.py'),training_ready=False)
 for variant in a.variants:
  ...
  for ci,clip in enumerate(clips):
   ...
   for path,view,sample in zip(paths,views,input_frames):
    ...
    kc,kp=transformed_intrinsics(calibrations[sample['camera_key']]['Intrinsics'],*sample['raw_size_wh'],cfg['size'],model.mhmr_img_res)
    view['camera_intrinsics']=torch.from_numpy(kc).float()[None]
    view['K_mhmr']=torch.from_numpy(kp).float()[None]
   records=[];current=0
   def callback(i,res,view):
    ...
    arrays['sensor_camera_intrinsics']=view['camera_intrinsics'].detach().cpu().numpy()
    arrays['sensor_K_mhmr']=view['K_mhmr'].detach().cpu().numpy()
