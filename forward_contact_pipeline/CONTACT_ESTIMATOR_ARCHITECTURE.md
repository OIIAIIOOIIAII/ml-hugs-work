...
> **全量训练状态补充（2026-09-18）**：`rich_full_contact_v1_seed42_v2` 已完成真实全量 RICH Stage-A 训练。它使用冻结 GUSH3R 的每人预测、64 个脚底 vertex、每 vertex 的16个预测 point-map 邻居、25个局部与16个全局 DINO RGB token、pose/query/物理相机条件；输出仅为每脚底 vertex 的 contact logit。GT body/contact 只用于将预测目标互斥地关联到标签及计算损失；没有作为模型输入。30 epoch内部开发验证的best是epoch5（all-candidate AP=.94421、F1=.86902、coverage=.89676），完整证据在`reports/rich_full_contact_v1_v2_20260918.md`。该运行不包含 Stage-B、修正量、LBS 回写或官方 test，因此不能作为端到端可部署结论。
