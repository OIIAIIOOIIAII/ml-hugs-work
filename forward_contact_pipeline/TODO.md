...
- [x] **A5.30 冻结 checkpoint 细粒度 internal validation（2026-09-18 完成）**：epoch-5 `best.pt` 已在 2,426 个 ParkingLot2 internal-validation 分片完整复评，74,264 candidates、9,505,792 有效顶点。复现 all-candidate AP=.944209/F1=.869023、matched AP=.974443/F1=.921677、coverage=.896760；与训练 history 差异小于2e-6。新增可复现评测器`scripts/evaluate_rich_full_contact.py`，逐片验证 plan/cache/contract，输出按左右脚、23序列、4主体、阈值和校准分箱的机器可读统计。7,667 个未唯一关联候选占顶点10.32%、正例率74.87%，是 all-candidate recall 下降的主因；先按关联失败原因分桶，不能仅拿 matched 结果调参。详见`reproducibility/rich_full_contact_v1_seed42_v2/internal_validation_detailed.md`；它仍不是 official val/test。
...
- [~] **A5.26 分离粗几何初始化与小幅接触控制**：改进图像/局部场景条件与顶点/关节几何监督，建立置信度/回退和更广训练覆盖；新的序列/参与者留出确认集，避免在已看过的12片段上继续选方案。当前失败ridge不部署，正式contact训练与全量缓存门槛仍未解除。
- [x] **A5.26a 非线性视觉/几何损失验证**：3配置×2种子完成，GT仅监督；可微FK/梯度/零修正验证通过。新6序列camera0脚底10.02→22.47cm，泛化失败，未部署。原camera3多人覆盖失败保留；相机K解析平移未解决脚底。详见本轮报告。
- [x] **A5.27 扩大覆盖并加强训练内留出**：528→1360样本、5→9人物，MLP/骨架图×2预算×2种子，8组完成；4新序列全部可测，0/4通过。数据/结构有局部收益但无可靠整体改善，完整负结果见本轮报告。
- [ ] **A5.28 重构信息与目标条件**：优先目标人物关联，恢复多人画面中的有效监督；将相机条件真正接入几何初始化（当前lighter人体分支忽略K），检验输入扰动可达性；加入脚部稠密图像patch/可见性与因果时序，分离整体尺度与关节误差。预留新确认数据；当前4片此后只可作开发或历史诊断，不再称全新确认。
