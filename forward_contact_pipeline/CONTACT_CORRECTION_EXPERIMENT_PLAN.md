...
## 2026-09-16 数据覆盖×关节图结构对照（完成，精度未通过）

本轮8组训练和4段新序列确认已完成，详见[`reports/rich_coverage_kinematic_20260916.md`](reports/rich_coverage_kinematic_20260916.md)。small528样本/5人物扩至1360样本/9人物，均1200更新×batch32；内部选择96帧、确认64帧。选中的expanded MLP seed7使原相机全身39.29→19.69cm，但对齐脚底15.84→21.40cm，所有模型0/4通过。图结构有局部收益但跨指标/种子不稳定，不部署，正式contact仍gated。

已重新定位：65/140新增片段因多人输出排除；420帧对应审计中180多人帧均只有一个头在目标人体框内，提示可用数据可能因匹配策略损失（不是已验证身份）。新增fit深度偏差+40cm、内部留出+302cm、确认−25cm，条件偏移明显。另完成192帧真实K输入干预：K全部改变，人体参数/query/网格逐值完全不变；源码确认当前lighter人体分支不使用传入K。停止当前全局query残差族的盲目扫参，优先目标关联、真正接通相机条件、脚部局部视觉与整体/局部误差分解。以下保留本轮冻结设计。

- 冻结37个单人物标注序列的2240计划帧；拟合人物9位，003/020两位人物及camera4完全留出，只在该留出集选模。选机位前的metadata检查发现若干指定机位无有效连续帧，计划最终使用fit的前两个可用非4机位；失败草稿保留，未读取新几何误差。
- 小预算为旧缓存剔除003/020和camera4，大预算为该小预算与新增fit样本去重并集。MLP与共享关节图网络各两种预算、两种种子；每组1200更新、相同几何损失，避免把更多训练步数混作数据量收益。共8组训练，零修正/限强度回退保留。
- 4个新的ParkingLot2序列，每段16帧，与训练窗口一致；锁定模型后才评价。配置`configs/experiments/rich_coverage_kinematic_v1.json`。若扩数和结构均无跨种子稳定脚底收益，停止当前全局query残差系列，重新定位信息/目标，不放宽2cm标准。
...
实际训练832图像样本/553不同人体帧/17序列/7参与者；16个检测数量不唯一的训练片段排除并保留失败记录。
零修正CUDA/TF32复现max-abs=0。全部残差校正使12确认片段camera-local对齐脚底8.83→13.72cm，
训练内选择的半强度旋转修正仍为11.36cm，均0/12通过。4/8/17序列在未见场景脚底12.70/10.92/12.93cm，
不能证明更多数据会持续改善，也不能排除更大、更多样本的帮助。
真值全部残差的无界诊断达到脚底0.54cm/12片段通过；当前限幅下4.92cm/4片段通过，表明小修正假设也有局限。
该上限不是部署结果，失败模型不采用，正式contact门槛保持。后续区分粗几何初始化与小幅接触控制，
以几何损失和图像/场景证据改进可学习性，并使用新的留出确认集；本次追加分量实验属于复用确认集的探索。

用户明确要求解决精度、多做实验，并追问训练数据不足的可能性。本轮允许有界的真实监督几何校正研究：
冻结GUSH3R，仅训练小型参数残差模型，监督来自RICH身体真值。它与后续dense contact估计器不同；
原始GUSH3R误差不是“已训练接触修复器”的误差。2cm几何准入门槛和正式contact主表限制保留。

- 已完成5种开发前处理/记忆对照、12个预先冻结新片段上的4种确认。geometry-only基线与原32帧网格max-abs差为0；
  原始/原生896人体输入的脚底median-of-clip-medians分别8.52/8.19cm，没有片段同时满足body/sole 2cm。
  标定rectification需要真实传感器K（不是身体/接触标签），其中1/12片段检测/身份失败单列，不从分母消失。
  32过去帧上下文也未稳定改善。全身与脚底、原camera误差和诊断Sim3后的误差均保留。
- 本机GUSH3R/Human3R去除module包装前缀后的1631个共有权重张量逐值相等；直接切换同一几何权重不算独立模型对照。
- 小校正实验预先固定：17个train序列，68个16帧片段，共1088个计划帧；确认序列不进入训练。
  inner-train序列划分选择pose-only/pose+token PCA32和ridge系数1/10/100/1000。
  输出为body/root旋转、neutral形状、头部平移残差；旋转限幅0.35rad、beta分量2、平移不超过depth的10%。
  所有GT仅作训练/评测目标，预测只读当帧/过去前端结果；不是每帧GT拟合或最终低维脚部控制器。
- 学习曲线预先固定4/8/17训练序列×种子7/19/42；每个预算独立拟合特征统计，固定确认集只评估、不选超参。
  失败检测片段保留计数。比较原输出、train平均残差控制和学习模型，分别报告原camera及held-out诊断误差。
  不能仅以训练loss下降、单片段通过或增加数据带来的表观改善宣称问题解决。

协议与结果证据：`reports/rich_geometry_20260916/`；处理数据仍见`RICH_DATASET.md`。
...
**9月16日Git迁移同步**：用户要求同步本地仓库，本轮整理HUGS/contact/RICH源码、配置、测试与文档快照，并核验第三方固定版本和补丁。RICH数据、模型、凭据及运行产物不进入Git；新增审计覆盖元数据TSV和数据处理依赖。远端仍为既有公开仓库，未改变可见性；仓库专用部署密钥deploy已获授权，源码提交8ba0dcf已成功推送至origin/main。未启动真实训练。
