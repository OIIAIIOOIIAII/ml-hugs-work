...
| `runs/rich_full_contact_v1_seed42_v2/best.pt` | 6.3MB | 二进制 checkpoint 属运行产物；仓库应避免持续累积模型历史 | 当前 SHA256 `645ef6e02c075a4da5578bffd61bea0561f26cf5cb46f613cd76568dc5b87406`；可受控迁移或重新训练 | 复用最佳模型需要 |
...
传统 HUGS/NeuMan 的 output、渲染图、训练 checkpoint 和历史 run 同样不进 Git；其代码、配置、文档和关键结论已同步。新机器只在明确需要复现该条支线时，再按相应数据与环境说明获取资产。
...
普通 Git 承载代码、文档、配置、patch、manifest 与小型数值证据。受许可的数据和模型应从官方来源或受控 NAS 获取；可再生 cache 和 checkpoint 只在受控存储传递。不要为“迁移方便”把 RICH、SMPL/SMPL-X、权重、cookie 或私钥上传到 Git/LFS、公开 Hugging Face 或公开网盘。
